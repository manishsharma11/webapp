...
public class CacheMonitor {
    ...
    @Around("execution(* com.main.sts.dao.*.*(..))")
    public Object log(ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (hit1 > hit0) {
            LOG.debug(String.format("CACHE HIT; Ratio=%s; Signature=%s#%s()", NF.format(ratio), pjp.getTarget()
                    .getClass().getName(), pjp.getSignature().toShortString()));
        } else if (miss1 > miss0) {
            LOG.debug(String.format("CACHE MISS; Ratio=%s; Signature=%s#%s()", NF.format(ratio), pjp.getTarget()
                    .getClass().getName(), pjp.getSignature().toShortString()));
        } else {
