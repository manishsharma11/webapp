...
<script type="text/javascript">
	...
	var ids = [];
			 ...
			 //alert($('input.uniform', this).is(':checked'));		
					if($('input.uniform', this).is(':checked')){					
				       ...
				       var stuHiddenId = $('#stuHiddenId',this).val();	
				        var sid=$('#sid',this).serialize();
				        var sname=$('#sname',this).serialize();
				        sid=$.trim(sid);
				        sname=$.trim(sname);
				     // alert(sid);
		...
		$.ajax({
			...
			url:"/sts/school_admin/persons/removeGuardian",
			data:'stuIds='+ids+'&sid='+$('#sid').serialize()+'&sname='+$('#sname').serialize(),
			dataType:"text",
			type:"POST",
			...
			<div class="container">
				...
				<div class="page-header">
					...
					<div class="page-title">
						<h3>Guardians</h3>
						<span>Update,Delete Guardians</span>
					</div>
					<br>
					...
					<div class="col-sm-6 col-md-3">
						<div class="statbox widget box box-shadow">
							<div class="widget-content">
								...
								<div class="title">Total Guardians</div>
								<div class="value">
				...
				<div class="col-md-8">
					...
					<a href="/sts/school_admin/persons/students" class="btn btn-primary"> <i
						class="icon-backward"></i> <span>Back</span>
					</a>				
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4>
									<i class="icon-reorder"></i>Guardians
								</h4>
								<div class="toolbar no-padding">
								Commuter Name: <h4>${sname }</h4>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								Parent Name: <h4>${fname }</h4>
									<div class="btn-group">
									...
									<tbody>
										...
										<c:choose>
											...
											<td class="checkbox-column">
												...
												<input type="hidden" value="${studentid}" id="sid" name="sid">
												<input type="hidden" value="${sname}" id="sname" name="sname">
												<input type="hidden" value="${fname}" id="fname" name="fname">
											</td>
											<td>
											...
											<td><c:out value="${student.relation }"></c:out></td>
											<td><c:out value="${student.mobile_number }"></c:out></td>	
											 
											<td><c:out value="${student.email }"></c:out></td>
											<td><c:out value="${student.getEmail_alert().getRegularities() }"></c:out></td>
											<td><c:out value="${student.getSms_alert().getRegularities() }"></c:out></td>
											<%-- <td><c:out value="${student.bus_licence_number_school }"></c:out></td>
											<td><c:out value="${student.stop_name_school }"></c:out></td>	 --%>
											<c:if test="${login_role  == 'ROLE_ADMIN' }">	
											<td><a href="/sts/school_admin/persons/students/updateguardian?id=${student.id}" class="btn btn-inverse"> <i
													class=" icon-edit"></i> <span>Update</span></a></td></c:if>
											<td><a href="/sts/school_admin/persons/students/viewdetailguardian?id=${student.id}" class="btn btn-info"> <i
													class="icon-fullscreen"></i> <span>Details</span></a></td>																				
											</tr>
										</c:forEach>
