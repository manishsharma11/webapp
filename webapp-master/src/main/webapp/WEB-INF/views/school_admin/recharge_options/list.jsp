		...
		$("td.checkbox-column").each(function(){
			// alert('foreach');								 					
					if($('input.uniform', this).is(':checked')){					
				       ...
				       var rfids = $('#rechargeIdHidden',this).val();				      
				       ids.push(rfids);				    	
				      // alert(ids);					    	
		}		
	});		
		$.ajax({
    		url : "removeAllBusesByBusIds",
    		type : "POST",
    		data : "busIds="
    				+ ids,
    		success : function(
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#search_button").click(function(){			
			var search_recharge=$("#search_recharge_id").val();						
			search_recharge=$.trim(search_recharge);			
			var searchOption=$("#search_searchOption").val();						
			var regx = /^[A-Za-z0-9]+$/;					
			if(search_recharge==""||!regx.test(search_recharge)){				
				...
				$("#rfid_number_status_success").hide();						
				 ...
				 $.ajax({				
					url:"${pageContext.request.contextPath}/school_admin/recharge_options/search",
					type:"POST",
					data:'search_recharge='+search_recharge+'&searchOption='+searchOption,
...
$(function(){
    $.contextMenu({
        ...
        callback: function(key, options) {
         ...
         if(key=="edit"){        	
             window.location="updaterecharge?id="+va;
         }
         if(key=="delete"){        	         	 
             window.location="removerecharge?id="+va;
         }
         if(key=="view"){        	         	 
...
$(function(){
    $.contextMenu({
        ...
        callback: function(key, options) {
         ...
         else if(key == "unassign_driver"){
        	 window.location="unassign_driver?id="+va;
         }
        },
        items: {
            "edit": {name: "<i class='icon-pencil'>&nbsp;&nbsp;&nbsp;Update</i><br>", icon: "edit1"},  
            "unassign_driver": {name: "<i class='icon-trash'>&nbsp;&nbsp;&nbsp;Unassign Driver</i><br>", icon: "delete"},
        }
    });
		...
		<div id="content">
			<div class="container">
				...
				<div class="page-header">
					...
					<div class="page-title">
						<h3>Recharge Options List</h3>
						<span>Create,Update,Delete Recharge Options</span>
					</div>
					<br>
					...
					<div class="col-sm-6 col-md-3">
						<div class="statbox widget box box-shadow">
							<div class="widget-content">
								...
								<div class="title">Total Recharge Options</div>
								<div class="value">
									<c:out value="${rechargeOptionsList.size()}"></c:out>
								</div>
							</div>
			...
			<!--	</c:if> -->
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4>
									<i class="icon-reorder"></i>Recharge Options List
								</h4>
								...
								<table
									...
									<thead>
										<tr>
											...
											<th>Number of Credits Offered</th>
											<th>Enabled</th>
											<th>Created At</th>
											<th>Updated At</th>
									...
									<tbody >
										<c:forEach items="${rechargeOptionsList}" var="rechargeOption" varStatus="index">
											<c:if test="${login_role  == 'ROLE_ADMIN' }">
												<c:if test="${rechargeOption.rechargeOption_allotted  != 0 }">
													<tr class="context-menu-two box menu-1" data-toggle="collapse">
														<td><input type="checkbox" class="uniform" name="n"
															disabled="disabled"> <input
															type="hidden" value="${rechargeOption.id}" id="rechargeIdHidden"></td>
												</c:if>
											</c:if>

											<c:if test="${login_role  == 'ROLE_ADMIN' }">
												<c:if test="${rechargeOption.rechargeOption_allotted  == 0 }">
													<tr class="context-menu-one box menu-1" data-toggle="collapse" >
														<td class="checkbox-column"><input type="checkbox"
															class="uniform" id="singleCheckId"> <input
															type="hidden" value="${rechargeOption.id}" id="rechargeIdHidden">
														</td>
												</c:if>
											</c:if>

											<c:if test="${login_role  ne 'ROLE_ADMIN' }">
												<tr class="context-menu-three box menu-1" data-toggle="collapse">
													<td class="checkbox-column"><input type="checkbox"
														class="uniform" id="singleCheckId"> <input
														type="hidden" value="${rechargeOption.id}" id="rechargeIdHidden">
													</td>
										...
										<c:out value="${bus.bus_licence_number}"></c:out> </a></td>
											...
											<td><c:out value="${rechargeOption.id}"></c:out></td>  
											<td><c:out value="${rechargeOption.recharge_amount}"></c:out></td>
											<td><c:out value="${rechargeOption.num_credits_offered}"></c:out></td>
											<td>
											<c:if test="${rechargeOption.enabled == true }">
													<span class="label label-success">Enabled</span>
												</c:if> 
											<c:if test="${rechargeOption.enabled == false }">
													<span class="label label-danger">Disabled</span>
												</c:if> 
											</td>									
											<td><c:out value="${rechargeOption.created_at}"></c:out></td>
										</c:forEach>
									</tbody>
								</table>
							</div>
				...
				<div class="alert alert-info fade in">
					<i data-dismiss="alert"></i> <strong>Info!</strong> Only Recharges Available to Commuters are enabled.
				</div>
			</div>
			<!-- /.container -->
					...
					<div class="form-group">
						...
						<div class="col-md-7 clearfix">
							<select name="searchOption" id="search_searchOption"
								...
								<option value="enabled">Enabled Recharges</option>
						...
						<br> <br> <label for="chosen1" generated="true"
							...
							class="required">*</span></label>
						<div class="col-md-6">
							<input type="text" name="search_recharge" id="search_recharge_id"
