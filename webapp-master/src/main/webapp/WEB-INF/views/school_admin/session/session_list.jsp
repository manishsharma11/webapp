		...
		$("td.checkbox-column").each(function(){
			// alert('foreach');								 					
					if($('input.uniform', this).is(':checked')){					
				       ...
				       var rfids = $('#busIdHidden',this).val();				      
				       ids.push(rfids);				    	
				      // alert(ids);					    	
		}		
	});		
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		...
		$("#search_button").click(function(){			
			var search_bus=$("#search_bus_id").val();						
			search_bus=$.trim(search_bus);			
			var searchOption=$("#search_searchOption").val();						
			var regx = /^[A-Za-z0-9]+$/;					
			if(search_bus==""||!regx.test(search_bus)){				
				...
				$("#rfid_number_status_success").hide();						
				 ...
				 $.ajax({				
					...
					type:"POST",
					data:'search_bus='+search_bus+'&searchOption='+searchOption,
...
$(function(){
    $.contextMenu({
        ...
        callback: function(key, options) {
         ...
         if(key=="edit"){        	
             window.location="updatebus?id="+va;
         }
         if(key=="delete"){        	         	 
			...
			<div class="container">
				...
				<div class="page-header">
					...
					<div class="page-title">
						<h3>Sessions</h3>
						<span>Create,Update,Delete Sessions</span>
					</div>
					<br>
					...
					<div class="col-sm-6 col-md-3">
						<div class="statbox widget box box-shadow">
							<div class="widget-content">
								...
								<div class="value">
									<c:out value="${session.size()}"></c:out>
								</div>
							</div>
				...
				<c:if test="${login_role  == 'ROLE_ADMIN' }">
					<div class="col-md-8">
						<a href="addsession" class="btn btn-primary"> <i class="icon-plus"></i>
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4>
									<i class="icon-reorder"></i>Sessions List
								</h4>
									...
									<c:set var="x" value="0"></c:set>
										<c:forEach items="${session }" var="session">
											...
											<c:if test="${login_role  == 'ROLE_ADMIN' }">
													...
													<tr class="context-menu-one box menu-1" data-toggle="collapse" >
														<td class="checkbox-column"><input type="checkbox"
															...
															type="hidden" value="${session.id}" id="busIdHidden">
														</td>
											...
											<c:out value="${x }" ></c:out></td>
											 ...
											 <td><c:out value="${session.session_name}"></c:out></td>  
											<td><c:out value="${session.session_type}"></c:out></td>
											<td><fmt:formatNumber type="number" minIntegerDigits="2"
													maxIntegerDigits="2" value="${session.starting_time_hours}" />
												<c:out value=":" /> <fmt:formatNumber type="number"
													minIntegerDigits="2" maxIntegerDigits="2"
													value="${session.starting_time_minutes}" /></td>
											<td> <fmt:formatNumber type="number" minIntegerDigits="2" maxIntegerDigits="2" value="${session.end_time_hours }"/>
													<c:out value=":"></c:out><fmt:formatNumber type="number" minIntegerDigits="2" maxIntegerDigits="2" value="${session.end_time_minutes }"></fmt:formatNumber>
											</td>
											</tr>
				...
				<div class="alert alert-info fade in">
					<i data-dismiss="alert"></i> <strong>Info!</strong> Sessions are usually morning and afternoon in schools.
				</div>
			</div>
			<!-- /.container -->
					...
					<div class="form-group">
						...
						<div class="col-md-7 clearfix">
							<select name="searchOption" id="search_searchOption"
								...
								<option value="bus_licence_number">Session Name</option>
								<option value="driverId">Session Type</option>
